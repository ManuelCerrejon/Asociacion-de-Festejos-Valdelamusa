...
export function PostCard({
  ...
  const card = (
    <article className="group h-full overflow-hidden rounded-lg border border-azul-noche/8 bg-white shadow-sm transition duration-300 hover:border-grana/25 hover:shadow-xl hover:shadow-azul-noche/10">
      <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-azul-noche/8 to-grana/8">
        {image ? (
          <Image
            src={image}
            alt=""
        ...
        <span className="absolute bottom-3 left-3 rounded-full bg-white/90 px-3 py-1 text-xs font-black uppercase tracking-wider text-grana shadow-sm">
          {category}
        </span>
      </div>
      <div className="p-4 sm:p-5">
        ...
        <h3 className="mt-4 text-lg font-black leading-tight text-azul-noche sm:text-xl">
          {title}
        </h3>
